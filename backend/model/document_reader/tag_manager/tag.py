...
class Tag:
    ...
    def __init__(self, tag_name, tag_color):
        ...
        if not tag_name:  # Check if empty after stripping
            raise InvalidNameException("Tag", "Tag name cannot be empty or whitespace only")
    ...
    def rename_tag(self, name):
        ...
        :type name: str
        :raises InvalidNameException: If the new name is invalid
        """
        if not name or not isinstance(name, str):
            raise InvalidNameException("Tag", "Tag name must be a non-empty string")
        
        name = name.strip()
        if len(name) < InvalidNameException.MIN_TAG_NAME_LENGTH:
            raise InvalidNameException("Tag", f"Tag name must be at least {InvalidNameException.MIN_TAG_NAME_LENGTH} character long")
        
        if len(name) > InvalidNameException.MAX_TAG_NAME_LENGTH:
            raise InvalidNameException("Tag", f"Tag name cannot exceed {InvalidNameException.MAX_TAG_NAME_LENGTH} characters")
