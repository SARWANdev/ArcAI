...
def test_remove_tag_not_found(document_service):
    with patch.object(document_service.document_repository, 'get_by_document_id', return_value=None):
...
def test_get_project_documents_found(document_service, fake_document_data):
    with patch.object(document_service.document_repository, 'get_documents_by_project', return_value=[{'_id': 'docid'}]), \
         patch.object(document_service, 'get_document', return_value=MagicMock()):
        docs = document_service.get_project_documents('proj1')
        assert isinstance(docs, list)
        assert len(docs) == 1

def test_get_project_documents_none(document_service):
    with patch.object(document_service.document_repository, 'get_documents_by_project', return_value=None):
        docs = document_service.get_project_documents('proj1')
        assert docs is None
